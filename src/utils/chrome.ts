...
export const getIframeAppLevel = (mapping: object = {}): Promise<FinalTreeData[]> => new Promise<FinalTreeData[]>((resolve) => {
  chrome.devtools.inspectedWindow.eval(
    `JSON.stringify(
      function () {
          function buildIframeAppHierarchy(node = document.body) {
              let hierarchy = {};
              const reg = /^iframe/u;
              let dealNode = node;
              if (node.tagName.toLowerCase() == 'iframe') {
                  try {
                      const iframe = document.getElementById(node.getAttribute('id'));
                      dealNode = iframe.contentDocument.body || iframe.contentWindow.document.body;
                  } catch (e) {
  
                  }
              }
              dealNode.childNodes.forEach((childNode) => {
                  if (childNode.nodeType === Node.ELEMENT_NODE) {
                      if (reg.test(childNode.tagName.toLowerCase())) {
                          const childHierarchy = buildIframeAppHierarchy(childNode);
                          const name = childNode.getAttribute('id') || 'unnamed';
                          hierarchy[name] = {
                          ...
                          hierarchy = {
                              ...hierarchy,
                              ...buildIframeAppHierarchy(childNode),
                          };
                      }
                  }
              });
          ...
          return buildIframeAppHierarchy();
      }()
  )`,
    (res: string) => {
      console.log('getIframeAppLevel', JSON.parse(res));
      const treeData = objectToArray(JSON.parse(res) as OneTreeData, mapping);
      resolve(treeData);
    },
  );
